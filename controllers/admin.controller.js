...
// UPDATED: Bitrix24 user sync on create/update — NO DB storage of bitrix_user_id
...
} from "../services/bitrix24.service.js";
...
export const getAnalytics = async (req, res) => {
  ...
  const userStats = await pool.query(
    `SELECT COUNT(*) as total_users FROM users WHERE 1=1 ${companyFilter}`,
    params,
  );

  const locationStats = await pool.query(
    `SELECT COUNT(*) as total_logs FROM location_logs WHERE 1=1 ${companyFilter}`,
    params,
  );
  ...
  const coveragePercent =
    totalClients > 0 ? ((withCoords / totalClients) * 100).toFixed(1) : 0;
  ...
  const topAreas = await pool.query(
    `SELECT pincode as area, COUNT(*) as clients
    FROM clients
    WHERE pincode IS NOT NULL ${companyFilter}
    GROUP BY pincode
    ORDER BY clients DESC
    LIMIT 5`,
    params,
  ...
  const recentActivity = await pool.query(
    `SELECT
      (SELECT COUNT(*) FROM meetings WHERE created_at >= NOW() - INTERVAL '30 days' ${companyFilter}) AS meetings_last_month,
      (SELECT COUNT(*) FROM trip_expenses WHERE created_at >= NOW() - INTERVAL '30 days' ${companyFilter}) AS expenses_last_month,
      (SELECT COUNT(*) FROM clients WHERE created_at >= NOW() - INTERVAL '30 days' ${companyFilter}) AS new_clients_last_month`,
    params,
  );
...
export const getUserLocationLogs = async (req, res) => {
  ...
  if (req.isSuperAdmin) {
    query = `SELECT id, latitude, longitude, accuracy, activity, battery, notes, pincode, timestamp
             FROM location_logs WHERE user_id = $1
    ...
    query = `SELECT id, latitude, longitude, accuracy, activity, battery, notes, pincode, timestamp
             FROM location_logs WHERE user_id = $1 AND company_id = $2
...
export const getClockStatus = async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT timestamp FROM location_logs WHERE user_id = $1 ${companyFilter}
  ...
  res.json({ clocked_in: diffMinutes <= 5, last_seen: lastSeen.toISOString() });
};
...
export const getExpensesSummary = async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT u.id, COALESCE(SUM(e.amount_spent), 0) AS total_expense
    FROM users u
    LEFT JOIN trip_expenses e ON e.user_id = u.id ${!req.isSuperAdmin ? "AND e.company_id = $1" : ""}
    ${companyFilter}
...
export const getUserMeetings = async (req, res) => {
  ...
  const totalCountResult = await pool.query(
    `SELECT COUNT(*) FROM meetings m WHERE m.user_id = $1 ${req.isSuperAdmin ? "" : "AND m.company_id = $2"}`,
    req.isSuperAdmin ? [userId] : [userId, req.companyId],
  );
  ...
  const result = await pool.query(
    `SELECT
       ...
       m.start_latitude AS "startLatitude", m.start_longitude AS "startLongitude",
       m.start_accuracy AS "startAccuracy",
       m.end_latitude AS "endLatitude", m.end_longitude AS "endLongitude",
       m.end_accuracy AS "endAccuracy",
     ...
     FROM meetings m
     LEFT JOIN clients c ON m.client_id = c.id
     WHERE m.user_id = $1 ${companyFilter}
     ORDER BY m.start_time DESC LIMIT $2 OFFSET $3`,
    params,
  );
...
export const getUserExpenses = async (req, res) => {
  ...
  const logsResult = await pool.query(
    `SELECT * FROM trip_expenses WHERE user_id = $1 ${companyFilter}
     ORDER BY travel_date DESC
     LIMIT $${req.isSuperAdmin ? 2 : 3} OFFSET $${req.isSuperAdmin ? 3 : 4}`,
    req.isSuperAdmin ? [userId, limit, offset] : [userId, req.companyId, limit, offset],
  );
  ...
  for (const expense of expenses) {
    if (expense.isMultiLeg) {
      ...
      expense.legs = legsResult.rows.map((row) => ({
        id: row.id,
        expenseId: row.expense_id,
        legNumber: row.leg_number,
        startLocation: row.start_location,
...
export const getUserDetails = async (req, res) => {
  ...
  const result = await pool.query(
    `SELECT u.id, u.email, u.is_admin, u.is_super_admin, u.created_at, u.pincode,
            u.company_id,
            p.full_name, p.department, p.work_hours_start, p.work_hours_end,
            c.name as company_name, c.subdomain as company_subdomain
     FROM users u
...
// ─────────────────────────────────────────────────────────────
// CREATE USER — syncs to Bitrix24 (fire-and-forget, no DB storage)
// ─────────────────────────────────────────────────────────────
export const createUser = async (req, res) => {
  const {
    email,
  ...
  // ── Bitrix24 sync — fire-and-forget, response already sent ──
  setImmediate(async () => {
    try {
      await createBitrixUser({ email, fullName, department });
    } catch (err) {
      console.error(`❌ [Admin] Bitrix sync failed for new user ${email}:`, err.message);
    }
  });

  res.status(201).json({
    message: "UserCreated",
    user: { ...user, full_name: fullName, department },
  });
};

// ─────────────────────────────────────────────────────────────
// UPDATE USER — looks up Bitrix user by email, then updates
// ─────────────────────────────────────────────────────────────
export const updateUser = async (req, res) => {
  const { userId } = req.params;
  const { email, fullName, department, workHoursStart, workHoursEnd, isAdmin } = req.body;
  ...
  const companyFilter = req.isSuperAdmin ? "" : "AND company_id = $2";
  const checkParams = [userId];
  if (!req.isSuperAdmin) checkParams.push(req.companyId);
  ...
  // ── Bitrix24 sync — fire-and-forget, response already sent ──
  // Use the current (or updated) email to look up the Bitrix user, then update.
  const resolvedEmail = email || userCheck.rows[0].email;

  setImmediate(async () => {
    try {
      const bitrixUser = await findBitrixUserByEmail(resolvedEmail);
      if (bitrixUser) {
        await updateBitrixUser(bitrixUser.ID, { email: resolvedEmail, fullName, department });
      } else {
        // User not in Bitrix yet — create them now (idempotent: reuses if found)
        await createBitrixUser({ email: resolvedEmail, fullName, department });
      }
    } catch (err) {
      console.error(`❌ [Admin] Bitrix sync failed on updateUser ${userId}:`, err.message);
    }
  });

  res.json({
    message: "UserUpdated",
    user: { id: userId, email, ...profileResult.rows[0] },
  });
};
