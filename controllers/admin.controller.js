...
import { pool } from "../db.js";
import bcrypt from "bcryptjs";
import { createEmployeeTask } from "../services/bitrix24.service.js";
...
// Create user (admin version)
export const createUser = async (req, res) => {
  const { email, password, fullName, department, workHoursStart, workHoursEnd, isAdmin = false, phone } = req.body;
  ...
  // ── Bitrix24 Integration: create a task for the new employee ──────────────
  // Fire-and-forget — we never await this so it never blocks the API response.
  // Errors are caught inside createEmployeeTask and only logged, not thrown.
  createEmployeeTask({
    id:        user.id,
    email:     user.email,
    ...
    console.error("❌ [Bitrix24:EmployeeTask] Unhandled rejection (createUser):", err.message);
  });
  // ─────────────────────────────────────────────────────────────────────────
