...
// controllers/clients.controller.js
// UPDATED: All queries now filter by company_id
// UPDATED: Auto-creates Bitrix24 Company on every new client (non-blocking)
...
export const createClient = async (req, res) => {
  const { name, email, phone, address, city, latitude, longitude, status, notes } = req.body;
  ...
  // ─────────────────────────────────────────────────────────
  // BITRIX24 AUTO-SYNC (non-blocking)
  // Fires after the DB insert succeeds and the response has
  // already been queued.  A Bitrix24 failure will NEVER break
  // the GeoTrack client creation flow.
  // ─────────────────────────────────────────────────────────
  createBitrixCompany({
    id:      newClient.id,
    name:    newClient.name,
    ...
    city:    city || null,          // pass-through from request body
    pincode: newClient.pincode,
  }).catch((err) => {
    // Belt-and-suspenders: createBitrixCompany already swallows errors,
    // but just in case an unhandled rejection leaks out.
    console.error("❌ [Bitrix24] Unhandled rejection in createBitrixCompany:", err.message);
  });
