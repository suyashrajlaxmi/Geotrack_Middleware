...
//   const { name, email, phone, address, latitude, longitude, status, notes, pincode: bodyPincode } = req.body;

//   if (!name) {
//     return res.status(400).json({ error: "ClientNameRequired" });
//   }

//   // Priority: 1) pincode typed by user, 2) derived from lat/lng, 3) null
//   let pincode = bodyPincode ? String(bodyPincode).trim() : null;
//   if (!pincode && latitude && longitude) {
...
//   // Priority: 1) pincode typed by user, 2) derived from lat/lng, 3) keep existing (COALESCE in SQL)
//   let pincode = bodyPincode ? String(bodyPincode).trim() : null;
//   if (!pincode && latitude && longitude) {
//     pincode = await getPincodeFromCoordinates(latitude, longitude);
//   }
...
//      RETURNING ${CLIENT_SELECT_FIELDS}`,
//     [name, email || null, phone || null, address || null, latitude || null, longitude || null, status, notes || null, pincode, req.params.id, req.companyId]
//   );
...
//   console.log(`✅ Client updated: ${name} (Pincode: ${pincode || 'preserved'})`);
...
import { getCoordinatesFromPincode, getCoordinatesFromAddress, getPincodeFromCoordinates } from "../services/geocoding.service.js";
import { startBackgroundGeocode } from "../utils/geocodeBatch.js";
import { createBitrixCompany } from "../services/bitrix24.service.js";
...
export const createClient = async (req, res) => {
  const { name, email, phone, address, latitude, longitude, status, notes, pincode: bodyPincode, city } = req.body;
  ...
  // ── Bitrix24 sync (non-blocking) ──────────────────────────────
  // Fire-and-forget: GeoTrack responds immediately, Bitrix runs in background.
  // A failure here never breaks the GeoTrack response.
  createBitrixCompany({ ...newClient, city: city || null }).catch((err) => {
    console.error("❌ [Bitrix24] Background sync failed:", err.message);
  });
