...
export const userManagementCreate = async (req, res) => {
  ...
  try {
    ...
    // ── Bitrix24: fire-and-forget task creation for new employee ─────
    console.log(`🔥 [Bitrix24:EmployeeTask] Triggering task for new user: ${email}`);
    createEmployeeTask({
      id:        user.id,
      email:     user.email,
      ...
      console.error("❌ [Bitrix24:EmployeeTask] Unhandled rejection (userManagementCreate):", err.message);
    });
    // ────────────────────────────────────
    res.status(201).json({ message: "UserCreated", user: { ...user, full_name: fullName, department } });
  } catch(e) {
