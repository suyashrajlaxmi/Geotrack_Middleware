...
function renderView(filename, data = {}) {
  ...
  // Step 1: Replace {{key}} template placeholders
  html = Object.entries(data).reduce(
    (h, [k, v]) => h.replaceAll(`{{${k}}}`, String(v ?? "")), html
  );
  // Step 2: Inject window globals BEFORE first <script> tag
  // This guarantees GT_MID is available even if template replacement fails
...
// ── Config helpers — NOW stores company_id directly ───────────
function loadConfig() {
...
// Legacy: keep email for backward compat
...
// ── Helper: get company_id for a member_id ────────────────────
// Auto-resolves with NO login, NO picker, NO email required.
// Priority: 1) cached in config  2) legacy email  3) first company in DB
async function resolveCompany(memberId) {
  // ALWAYS find the company with the most users first — this is the definitive "real" company
  // Then compare with cached value; if cached differs → override with the correct one
  try {
    ...
    if (best.rows.length && +best.rows[0].user_count > 0) {
      ...
      // If cached is different from largest → override (fixes stale wrong-company mapping)
  ...
  // Fallback: use cached if DB query failed
  const cached = getPortalCompany(memberId);
  if (cached?.company_id) return cached;

  // Last resort: legacy email lookup
...
async function fetchDashData(memberId, company_id) {
  ...
  const [cStats, mStats, eStats, rClients, rMeetings, topC, topPins, allUsers, performers, analytic, planRes] = await Promise.all([
    ...
    // All users — for correct team headcount (not capped at 5)
    pool.query(`SELECT u.id FROM users u WHERE u.company_id=$1`, [company_id]).catch(()=>({rows:[]})),
    // Top performers — with meeting & client counts for performer cards
    pool.query(`
...
// ── App Launcher — GET/POST /bitrix/app ───────────────────────
// No login. If company is configured → show dashboard.
// If not → show company picker (dropdown of all companies, no password).
export const appLauncher = async (req, res) => {
  try {
    ...
    // Always auto-resolve company — no login, no picker, no user action needed
    const company = await resolveCompany(member_id);

    if (!company?.company_id) {
      // Only shows if DB has zero companies — edge case
    ...
    // Always go straight to dashboard
    // NOTE: Bitrix admin login is available at /bitrix/login (optional — uses sessionStorage)
    // It does NOT gate access here — Bitrix24 iframe auth handles identity
    const html = renderView("dashboard.html", { member_id, DOMAIN });
...
// ── Select Company — POST /bitrix/select-company ─────────────
// Called by the picker form. Saves company_id to config, redirects to dashboard.
...
// ── App Data — GET /bitrix/data ───────────────────────────────
export const appData = async (req, res) => {
  res.setHeader("ngrok-skip-browser-warning", "true");
  try {
    // member_id may be empty — resolveCompany picks company with most users as fallback
    const member_id = req.query.member_id || req.body?.member_id || "";
...
export const resetCompany = async (req, res) => {
  ...
  try {
    const { member_id } = req.body;
    // member_id may be empty — resolveCompany handles it with fallback
...
export const mapLauncher = async (req, res) => {
  try {
    ...
    const html = renderView("map.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
export const mapData = async (req, res) => {
  ...
  try {
    ...
    const [cRows, mRows, agentRows, totalTrackedRows, totalClientsRow] = await Promise.all([
      // Clients with coordinates — include email for sidebar display
      pool.query(
      ...
      // Field visits (meetings) with GPS — used both for map pins AND quick_visits stats
      pool.query(
      ...
      // All company users + their most recent location log (if any)
      // Uses LEFT JOIN so users with no location data still appear as Offline
      pool.query(
        `SELECT DISTINCT ON (u.id)
         ...
         ORDER BY u.id, ll.timestamp DESC NULLS LAST`,
        [company_id]
      ).catch(() => ({ rows: [] })),
      // Total unique users ever tracked (all-time)
      pool.query(
      ...
      // Total clients (including those without GPS) — for "of X total" in Online Users card
      pool.query(
    ...
    const agents = agentRows.rows.map(a => ({
      ...
      // Online = last seen within 30 minutes
    ...
    // Build meetings array and derive quick_visits stats from it
    ...
    return res.json({
      ...
      agents,
      // Field visit stats derived from meetings (no separate quick_visits table needed)
      quick_visits: {
        total:      qvTotal,
        successful: qvSuccessful,
      },
      total_tracked: totalTracked,
      total_clients: parseInt(totalClientsRow.rows[0]?.total || 0),
      online_count: agents.filter(a => a.is_online).length,
      // Online clients = clients whose pincode matches any currently-online agent's pincode.
      // Per GeoTrack design: field agents see clients in their current pincode area.
      // Falls back to coordinate proximity (~5km) for clients/agents missing pincodes.
      online_clients: (() => {
        const onlineAgents = agents.filter(a => a.is_online);
        if (onlineAgents.length === 0) return 0;

        // Build set of online agent pincodes (from users.pincode via COALESCE)
        ...
        cRows.rows.forEach(c => {
          ...
          // Primary: pincode match
          if (cPin && onlinePincodes.has(cPin)) {
            matched.add(c.id);
            return;
          }

          // Fallback: coordinate proximity (~5km radius) for clients without pincode
          // or agents without pincode
...
// ── Legacy: keep these exports so old routes still compile ────
...
// ═══════════════════════════════════════════════════════════════
// JOURNEY TRACKING — GET /bitrix/journey-agents & /bitrix/journey-data
// ═══════════════════════════════════════════════════════════════

// ── Journey page launcher — GET /bitrix/journey ───────────────
export const journeyLauncher = async (req, res) => {
  try {
    ...
    const html = renderView("journey.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Agents list — GET /bitrix/journey-agents ──────────────────
// Returns all agents (profiles) in this company for the dropdown
export const journeyAgents = async (req, res) => {
  ...
  try {
    ...
    // All users in this company who have a profile
    const r = await pool.query(
    ...
    // Fallback: if no profiles table, try users directly
    if (!r.rows.length) {
      const r2 = await pool.query(
        `SELECT id AS user_id, email AS full_name, email
         FROM users WHERE company_id=$1 AND role != 'admin' ORDER BY email ASC`,
        [company_id]
      ).catch(() => ({ rows: [] }));
...
// ── Journey data — GET /bitrix/journey-data ───────────────────
// Returns location logs + meetings for a specific agent + date range
export const journeyData = async (req, res) => {
  ...
  try {
    ...
    // Add 1 day to end so it includes the full end day
    ...
    // ── 1. Location logs for this user in date range ──────────
    // Try multiple possible table/column names (graceful fallback)
    let logsRows = [];
    const logsQueries = [
      // Most likely schema based on screenshots
      `SELECT id, latitude, longitude, timestamp, pincode, accuracy
       FROM location_logs
       WHERE user_id = $1 AND timestamp >= $2 AND timestamp < $3
       ORDER BY timestamp ASC`,
      // Alternative: created_at instead of timestamp
      `SELECT id, latitude, longitude, created_at AS timestamp, pincode
       FROM location_logs
       WHERE user_id = $1 AND created_at >= $2 AND created_at < $3
       ORDER BY created_at ASC`,
      // Alternative: recorded_at
      `SELECT id, latitude, longitude, recorded_at AS timestamp
       FROM location_logs
    ...
    for (const q of logsQueries) {
      try {
        const r = await pool.query(q, [user_id, startDate, endDateStr]);
        logsRows = r.rows;
        break;
      } catch(e) { /* try next */ }
    }

    // ── 2. Meetings for this user in date range ───────────────
    let mtgRows = [];
    try {
      const r = await pool.query(
        `SELECT m.id, m.start_time, m.end_time, m.status, m.comments,
                m.start_latitude, m.start_longitude,
                m.distance_to_client,
                m.location_verified,
                c.name AS client_name,
        ...
        [user_id, company_id, startDate, endDateStr]
      );
      mtgRows = r.rows;
    } catch(e) {
      // Fallback: without some columns that may not exist
      try {
        const r = await pool.query(
          `SELECT m.id, m.start_time, m.end_time, m.status, m.comments,
                  m.start_latitude, m.start_longitude,
    ...
    // ── 3. Compute stats ──────────────────────────────────────
    const logsCount     = logsRows.length;
    const meetingsCount = mtgRows.length;
    const clientsVisited= mtgRows.filter(m => m.status === "COMPLETED").length;

    // Total distance: sum of haversine between consecutive GPS points
    ...
    // Duration: time between first and last log
...
// ═══════════════════════════════════════════════════════════════
// NEW: Clients + Client Services pages
// ═══════════════════════════════════════════════════════════════

// ── Clients Page Launcher — GET /bitrix/clients ───────────────
export const clientsLauncher = async (req, res) => {
  try {
    const DOMAIN    = req.query.DOMAIN    || req.body.DOMAIN    || "world.bitrix24.com";
    const member_id = req.query.member_id || req.body.member_id || "";
    res.setHeader("ngrok-skip-browser-warning", "true");
    console.log(`🔍 clientsLauncher: method=${req.method} member_id=${member_id} domain=${DOMAIN}`);
    const company = await resolveCompany(member_id);
    if (!company?.company_id) {
      return res.redirect(`/bitrix/app?DOMAIN=${DOMAIN}&member_id=${member_id}`);
    }
    const html = renderView("clients.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Clients Data — GET /bitrix/clients-data ───────────────────
export const clientsData = async (req, res) => {
  ...
  try {
    ...
    const synced    = loadSynced();
    const syncedIds = Object.keys((synced[member_id]?.clients) || {});
    // Simple safe query — no optional columns, no GROUP BY complexity
    ...
    // Last visit per client in a separate safe query
    const { rows: visitRows } = await pool.query(
      `SELECT client_id, MAX(start_time) AS last_visit
       FROM meetings WHERE company_id = $1 GROUP BY client_id`,
      [company_id]
    ).catch(() => ({ rows: [] }));
    ...
    const clients = clientRows.map(c => ({
      ...
      last_visit: visitMap[String(c.id)] || null,
      source:     null,
      synced:     syncedIds.includes(String(c.id)),
    }));
...
// ── Save Client — POST /bitrix/clients-save ───────────────────
...
// ── Client Services Page Launcher — GET /bitrix/client-services
export const servicesLauncher = async (req, res) => {
  try {
    // Read from BOTH query string (GET nav) and body (POST from Bitrix24)
    const DOMAIN    = req.query.DOMAIN    || req.body.DOMAIN    || "world.bitrix24.com";
    const member_id = req.query.member_id || req.body.member_id || "";
    res.setHeader("ngrok-skip-browser-warning", "true");
    console.log(`🔍 servicesLauncher: method=${req.method} member_id=${member_id} domain=${DOMAIN}`);
    const company = await resolveCompany(member_id);
    if (!company?.company_id) {
      return res.redirect(`/bitrix/app?DOMAIN=${DOMAIN}&member_id=${member_id}`);
    }
    const html = renderView("client.services.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Services Data — GET /bitrix/services-data ─────────────────
// ── Auto-create client_services table if it doesn't exist ─────
...
export const servicesData = async (req, res) => {
  ...
  try {
    ...
    // Auto-create table on first use — safe if already exists
...
// ── Save Service — POST /bitrix/services-save ─────────────────
    ...
    if (id) {
      await pool.query(
        `UPDATE client_services
         SET service_name=$1, description=$2, client_id=$3, price=$4,
             status=$5, start_date=$6, expiry_date=$7, updated_at=now()
         WHERE id=$8 AND company_id=$9`,
        [service_name, description||null, client_id, price||null,
         status||"active", start_date||null, expiry_date||null, id, company_id]
      );
      return res.json({ ok: true, action: "updated", id });
    } else {
      const r = await pool.query(
        `INSERT INTO client_services
           (company_id, client_id, service_name, description, price, status, start_date, expiry_date)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
         RETURNING id`,
        [company_id, client_id, service_name, description||null,
         price||null, status||"active", start_date||null, expiry_date||null]
      );
      return res.json({ ok: true, action: "created", id: r.rows[0].id });
    }
  } catch(e) {
...
// ═══════════════════════════════════════════════════════════════
// TEAM ACTIVITY — all handlers (FIXED v2)
// ═══════════════════════════════════════════════════════════════

// ── Ensure is_paid column exists on trip_expenses ──────────────
async function ensureIsPaidColumn() {
  // Explicit check first — ADD COLUMN IF NOT EXISTS silently fails on some PG setups
  try {
    const chk = await pool.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name='trip_expenses' AND table_schema='public' AND column_name='is_paid'
    `);
    if (!chk.rows.length) {
    ...
    console.warn('⚠️  ensureIsPaidColumn:', e.message);
  }
}

// ── Ensure company_id columns exist on all activity tables ───
async function ensureCompanyIdColumns() {
  try {
    await pool.query(`ALTER TABLE location_logs  ADD COLUMN IF NOT EXISTS company_id INTEGER`);
  } catch(e) { /* already exists */ }
  try {
    await pool.query(`ALTER TABLE meetings        ADD COLUMN IF NOT EXISTS company_id INTEGER`);
  } catch(e) { /* already exists */ }
  try {
    await pool.query(`ALTER TABLE trip_expenses   ADD COLUMN IF NOT EXISTS company_id INTEGER`);
  } catch(e) { /* already exists */ }
}

// ── Team Activity Launcher — GET|POST /bitrix/team-activity ───
export const teamActivityLauncher = async (req, res) => {
  ...
  try {
    ...
    const html = renderView("team.activity.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Team Data — GET /bitrix/team-data ─────────────────────────
// Returns users enriched with: is_clocked_in, last_seen_time, last_pincode,
// total_meetings, completed_meetings, total_expenses
export const teamData = async (req, res) => {
  ...
  try {
    ...
    // ── 1. Users + profiles ──────────────────────────────────
    // Ensure company_id columns exist before querying them
    await ensureCompanyIdColumns();

    // Primary approach: users directly scoped to this company
    let { rows: users } = await pool.query(
      `SELECT DISTINCT ON (u.id)
       ...
       LEFT JOIN profiles p ON p.user_id = u.id
       WHERE u.company_id = $1
       ORDER BY u.id, COALESCE(p.full_name, u.email) ASC`,
      [company_id]
    );

    // Fallback: if no users found via company_id, try activity-table UNION
    // (handles cases where company_id column exists but data predates multi-tenancy)
    if (!users.length) {
      try {
        const fallback = await pool.query(
          `SELECT DISTINCT ON (u.id)
           ...
           WHERE u.id IN (
             ...
             UNION
             SELECT DISTINCT user_id FROM meetings       WHERE company_id = $1
             UNION
             SELECT DISTINCT user_id FROM trip_expenses  WHERE company_id = $1
           )
           ORDER BY u.id, COALESCE(p.full_name, u.email) ASC`,
          [company_id]
        );
        ...
        console.warn('⚠️  teamData UNION fallback failed:', e2.message);
      }
    }

    if (!users.length) {
      return res.json({ company: company_name, users: [] });
    }

    const userIds = users.map(u => u.user_id);

    // ── 2. Last location log per user ────────────────────────
    // Use text cast to avoid UUID type issues
    const { rows: lastLogs } = await pool.query(
      `SELECT
         user_id::text,
         MAX(timestamp)                                      AS last_log_time,
         (array_agg(pincode ORDER BY timestamp DESC)
           FILTER (WHERE pincode IS NOT NULL))[1]           AS last_pincode
       FROM location_logs
    ...
    // ── 4. Expense totals per user ───────────────────────────
    const { rows: expRows } = await pool.query(
      `SELECT
         user_id::text,
         COALESCE(SUM(amount_spent), 0) AS total_expenses
       FROM trip_expenses
       WHERE user_id = ANY($1::uuid[])
       GROUP BY user_id`,
      [userIds]
    ...
    const enriched = users.map(u => {
      ...
      // Format last_seen as "Mar 12, 03:55 PM"
      ...
      return {
        ...
        // Fields matching team_activity.html expectations:
        is_clocked_in:      !!isActive,
        last_seen_time:     lastSeenTime,
        last_pincode:       log.last_pincode || u.user_pincode || null,
...
// ── Team Logs Launcher — GET|POST /bitrix/team-logs ───────────
export const teamLogsLauncher = async (req, res) => {
  ...
  try {
    ...
    // Guard: if user_id missing, bounce back to team-activity
    if (!user_id) {
      console.warn("⚠️  teamLogsLauncher: no user_id — redirecting to team-activity");
      return res.redirect(`/bitrix/team-activity?DOMAIN=${encodeURIComponent(DOMAIN)}&member_id=${encodeURIComponent(member_id)}`);
    }
    const html = renderView("team.logs.html", { member_id, DOMAIN, user_id, user_name });
    return res.send(html);
  } catch(e) {
...
// ── Team Logs Data — GET /bitrix/team-logs-data ───────────────
export const teamLogsData = async (req, res) => {
  ...
  try {
    ...
    // Validate UUID — a truncated/malformed UUID silently returns 0 rows
    const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    const cleanUID = String(user_id).trim();
    if (!uuidRe.test(cleanUID)) {
      console.error(`❌ teamLogsData: invalid UUID "${cleanUID}" (len=${cleanUID.length})`);
    ...
    // First try with company_id filter for data isolation
    ...
    // Fallback: if 0 rows, query without company_id filter
    // (handles data where company_id was null or column didn't exist yet)
    if (!logs.length) {
      const fallback = await pool.query(
        `SELECT
           id,
...
// ── Team Meetings Launcher — GET|POST /bitrix/team-meetings ───
export const teamMeetingsLauncher = async (req, res) => {
  ...
  try {
    ...
    const user_name = req.body?.user_name || req.query?.user_name || "User";
    if (!user_id) {
      console.warn("⚠️  teamMeetingsLauncher: no user_id — redirecting to team-activity");
      return res.redirect(`/bitrix/team-activity?DOMAIN=${encodeURIComponent(DOMAIN)}&member_id=${encodeURIComponent(member_id)}`);
    }
    const html = renderView("team.meetings.html", { member_id, DOMAIN, user_id, user_name });
    return res.send(html);
  } catch(e) {
...
// ── Team Meetings Data — GET /bitrix/team-meetings-data ───────
export const teamMeetingsData = async (req, res) => {
  ...
  try {
    ...
    const cleanUID = String(user_id).trim();
    if (!uuidRe.test(cleanUID)) {
      console.error(`❌ teamMeetingsData: invalid UUID "${cleanUID}" (len=${cleanUID.length})`);
...
// ── Team Expenses Launcher — GET|POST /bitrix/team-expenses ───
export const teamExpensesLauncher = async (req, res) => {
  ...
  try {
    ...
    const user_name = req.body?.user_name || req.query?.user_name || "User";
    if (!user_id) {
      console.warn("⚠️  teamExpensesLauncher: no user_id — redirecting to team-activity");
      return res.redirect(`/bitrix/team-activity?DOMAIN=${encodeURIComponent(DOMAIN)}&member_id=${encodeURIComponent(member_id)}`);
    }
    const html = renderView("team.expenses.html", { member_id, DOMAIN, user_id, user_name });
    return res.send(html);
  } catch(e) {
...
// ── Team Expenses Data — GET /bitrix/team-expenses-data ───────
// Docs: trip_expenses schema has amount_spent, distance_km, transport_mode, receipt_urls
// Paid/Pending = admin-side concept tracked via is_paid BOOLEAN added on first use
// Total   = SUM(amount_spent)          — user-entered manually in app
// Paid    = SUM(amount_spent) WHERE is_paid=true
// Pending = Total - Paid
// Avg     = Total / COUNT(*)
...
export const teamExpensesData = async (req, res) => {
  ...
  try {
    ...
    const cleanUID = String(user_id).trim();
    if (!uuidRe.test(cleanUID))
      return res.status(400).json({ error: "invalid_user_id", expenses: [], summary: {} });

    const company = await resolveCompany(member_id);
    if (!company?.company_id)
      return res.status(200).json({ error: "NO_COMPANY", expenses: [], summary: {} });
    const { company_id, company_name } = company;

    // Step 1: ensure is_paid column exists
    await ensureIsPaidColumn();

    // Step 2: discover ALL actual columns at runtime — never assume schema
    const { rows: colRows } = await pool.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_name = 'trip_expenses' AND table_schema = 'public'
    `).catch(() => ({ rows: [] }));
    const cols = new Set(colRows.map(r => r.column_name));
    console.log(`💰 trip_expenses cols: ${[...cols].sort().join(', ')}`);

    // Step 3: build a safe SELECT — plain string helper avoids template-literal nesting
    ...
    const selParts = [
      ...
      // receipt: original schema uses receipt_urls; some installs use receipt_images
      cols.has('receipt_images')
        ? 'COALESCE(e.receipt_images, ARRAY[]::text[]) AS receipt_images'
        : cols.has('receipt_urls')
          ? 'COALESCE(e.receipt_urls, ARRAY[]::text[]) AS receipt_images'
          : 'ARRAY[]::text[] AS receipt_images',
      // is_paid cast to text to eliminate pg-driver boolean/string ambiguity
      // Check BOTH is_paid boolean AND payment_status string
      // React app marks paid via payment_status='PAID'; Bitrix uses is_paid=true
      (cols.has('is_paid') && cols.has('payment_status'))
        ...
        : cols.has('is_paid')
          ? "CASE WHEN e.is_paid IS TRUE THEN 'true' ELSE 'false' END AS is_paid"
          : cols.has('payment_status')
            ? "CASE WHEN UPPER(COALESCE(e.payment_status,''))='PAID' THEN 'true' ELSE 'false' END AS is_paid"
            : "'false' AS is_paid",
    ...
    // Step 4: fetch expenses — try with company_id filter, fallback without
    let expenses = [];
    if (cols.has('company_id')) {
      const r1 = await pool.query(
        `SELECT ${sel}
         ...
         WHERE e.user_id = $1::uuid AND e.company_id = $2
         ORDER BY e.created_at DESC LIMIT 200`,
        [cleanUID, company_id]
      ).catch(() => ({ rows: [] }));
      expenses = r1.rows;
    }
    // Fallback: no company_id (original schema has no company_id on trip_expenses)
    if (!expenses.length) {
      const r2 = await pool.query(
        `SELECT ${sel}
         FROM trip_expenses e
    ...
    console.log(`💰 teamExpensesData: user=${cleanUID} found=${expenses.length} expenses`);
    if (expenses.length) {
      console.log(`💰 is_paid sample:`,
        JSON.stringify(expenses.slice(0,3).map(e => ({ id:e.id, is_paid:e.is_paid, type:typeof e.is_paid }))));
    }

    // Step 5: fetch trip legs for multi-leg expenses
    const multiIds = expenses.filter(e => e.is_multi_leg === true || e.is_multi_leg === 'true').map(e => e.id);
    const legsMap  = {};
    if (multiIds.length) {
      const { rows: legs } = await pool.query(
        `SELECT expense_id, leg_number, start_location, end_location,
                distance_km, transport_mode, amount_spent, notes
         FROM trip_legs
         WHERE expense_id = ANY($1::uuid[])
         ORDER BY expense_id, leg_number`,
        [multiIds]
      ).catch(() => ({ rows: [] }));
      legs.forEach(l => { (legsMap[l.expense_id] = legsMap[l.expense_id] || []).push(l); });
    }

    // Step 6: normalise + enrich
    // is_paid arrives as string 'true'/'false' from CASE WHEN above
    // Handle all paid variants: boolean true, string 'true'/'t'/'1', payment_status 'PAID'
    const isPaid = v => v === true || v === 't' || v === 'true' || v === 1 || v === '1' ||
      (typeof v === 'string' && v.toUpperCase() === 'PAID');

    const enriched = expenses.map(e => ({
      ...e,
      receipt_images: Array.isArray(e.receipt_images) ? e.receipt_images : [],
      legs:           legsMap[e.id] || [],
      is_paid:        isPaid(e.is_paid),   // always plain JS boolean
      // travel_date stored as Unix-ms bigint — convert to readable date
      travel_date_display: e.travel_date
        ? new Date(
            typeof e.travel_date === 'string' && String(e.travel_date).length <= 13
              ? parseInt(e.travel_date)
              : e.travel_date
          ).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' })
        : '—',
    }));

    // Step 7: summary — all computed from enriched (normalised) data
    // Matches app docs: Total=SUM(amount_spent), Paid=SUM WHERE is_paid, Pending=Total-Paid
    ...
    return res.json({
      ...
      expenses: enriched,
      summary: {
        total_expenses: enriched.length,
        total_amount:   parseFloat(total.toFixed(2)),    // SUM(amount_spent)
        paid_amount:    parseFloat(paidAmt.toFixed(2)),  // SUM WHERE is_paid=true
        pending_amount: pendAmt,                          // total - paid
        total_distance: parseFloat(totalDist.toFixed(2)),// SUM(distance_km)
        paid_count:     paidItems.length,
        pending_count:  pendItems.length,
        avg_amount:     enriched.length
          ? parseFloat((total / enriched.length).toFixed(2))  // total / count
          : 0,
      }
    });
  } catch(e) {
...
// ── Team Expense Pay — POST /bitrix/team-expense-pay ──────────
export const teamExpensePay = async (req, res) => {
  res.setHeader("ngrok-skip-browser-warning", "true");
  try {
    const { member_id, expense_id } = req.body;
    if (!member_id || !expense_id) {
      return res.status(400).json({ error: "member_id and expense_id required" });
    }
    const company = await resolveCompany(member_id);
    if (!company?.company_id) return res.status(400).json({ error: "No company configured" });
    const { company_id } = company;
    ...
    // Check if company_id column exists on trip_expenses before using it
    const { rows: payColChk } = await pool.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_name='trip_expenses' AND table_schema='public' AND column_name='company_id'
    `).catch(() => ({ rows: [] }));
    ...
    if (hasCompanyCol) {
      updateResult = await pool.query(
        `UPDATE trip_expenses SET is_paid = true WHERE id = $1::uuid AND company_id = $2`,
        [expense_id, company_id]
      ).catch(() => ({ rowCount: 0 }));
    }
    // Fallback: update by id only (original schema — no company_id column)
    if (!updateResult.rowCount) {
      updateResult = await pool.query(
        `UPDATE trip_expenses SET is_paid = true WHERE id = $1::uuid`,
        [expense_id]
      ).catch(() => ({ rowCount: 0 }));
    }
...
// ── Team Expense Pay All — POST /bitrix/team-expense-pay-all ──
export const teamExpensePayAll = async (req, res) => {
  ...
  try {
    ...
    if (!member_id || !user_id) return res.status(400).json({ error: "member_id and user_id required" });
    const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (!uuidRe.test(String(user_id).trim())) return res.status(400).json({ error: "invalid_user_id" });
    const company = await resolveCompany(member_id);
    if (!company?.company_id) return res.status(400).json({ error: "No company configured" });
    const { company_id } = company;
    await ensureIsPaidColumn();
    ...
    if (hasCol) {
      result = await pool.query(
        `UPDATE trip_expenses SET is_paid = true WHERE user_id = $1::uuid AND (company_id = $2 OR company_id IS NULL) AND (is_paid IS NULL OR is_paid = false)`,
        [user_id, company_id]
      ).catch(() => ({ rowCount: 0 }));
    }
    if (!result.rowCount) {
      result = await pool.query(
        `UPDATE trip_expenses SET is_paid = true WHERE user_id = $1::uuid AND (is_paid IS NULL OR is_paid = false)`,
        [user_id]
      ).catch(() => ({ rowCount: 0 }));
    }
    await pool.query(`UPDATE trip_expenses SET payment_status = 'PAID' WHERE user_id = $1::uuid`,[user_id]).catch(()=>{});
    console.log(`✅ teamExpensePayAll: user=${user_id} updated=${result.rowCount}`);
...
// ── Debug Company — GET /bitrix/debug-company ────────────────
// Hit this to diagnose wrong company mapping in Bitrix iframe:
// /bitrix/debug-company?member_id=YOUR_MEMBER_ID
export const debugCompany = async (req, res) => {
  ...
  try {
    ...
    const resolved = member_id ? await resolveCompany(member_id) : null;
    const { rows: companies } = await pool.query(
      `SELECT c.id, c.name, COUNT(u.id) AS user_count
         FROM companies c LEFT JOIN users u ON u.company_id = c.id
         GROUP BY c.id, c.name ORDER BY user_count DESC`
    );
    const mappings = Object.entries(config).map(([mid, cfg]) => ({
      member_id: mid, company_id: cfg.company_id, company_name: cfg.company_name,
    }));
    return res.json({
      queried_member_id: member_id,
      cached_config:     cached,
      resolved_company:  resolved,
      all_companies:     companies,
      all_mappings:      mappings,
      fix: "POST to /bitrix/reset-company with {member_id} to clear stale cache, then reload."
    });
  } catch(e) { return res.status(500).json({ error: e.message }); }
};

// ── Debug Team — GET /bitrix/debug-team ───────────────────────
// Hit this URL to diagnose data issues:
// /bitrix/debug-team?member_id=YOUR_MEMBER_ID&user_id=USER_UUID
export const debugTeam = async (req, res) => {
  ...
  try {
    ...
    if (company?.company_id && user_id) {
      ...
      // Test 1: logs without company filter
      const r1 = await pool.query(
        `SELECT COUNT(*) AS cnt FROM location_logs WHERE user_id::text = $1`, [uid]
      ).catch(e => ({ rows: [{ cnt: "ERROR: " + e.message }] }));
      result.queries.logs_by_user_only = r1.rows[0].cnt;

      // Test 2: logs with company filter
      const r2 = await pool.query(
        `SELECT COUNT(*) AS cnt FROM location_logs WHERE user_id::text = $1 AND company_id = $2`, [uid, cid]
      ).catch(e => ({ rows: [{ cnt: "ERROR: " + e.message }] }));
      result.queries.logs_with_company = r2.rows[0].cnt;

      // Test 3: meetings
      const r3 = await pool.query(
        `SELECT COUNT(*) AS cnt FROM meetings WHERE user_id::text = $1`, [uid]
      ).catch(e => ({ rows: [{ cnt: "ERROR: " + e.message }] }));
      result.queries.meetings_by_user_only = r3.rows[0].cnt;

      // Test 4: meetings with company
      const r4 = await pool.query(
        `SELECT COUNT(*) AS cnt FROM meetings WHERE user_id::text = $1 AND company_id = $2`, [uid, cid]
      ).catch(e => ({ rows: [{ cnt: "ERROR: " + e.message }] }));
      result.queries.meetings_with_company = r4.rows[0].cnt;

      // Test 5: expenses
      const r5 = await pool.query(
        `SELECT COUNT(*) AS cnt FROM trip_expenses WHERE user_id::text = $1`, [uid]
      ).catch(e => ({ rows: [{ cnt: "ERROR: " + e.message }] }));
      result.queries.expenses_by_user_only = r5.rows[0].cnt;

      // Test 6: check company_id values in location_logs for this user
      const r6 = await pool.query(
        `SELECT DISTINCT company_id FROM location_logs WHERE user_id::text = $1 LIMIT 5`, [uid]
      ).catch(e => ({ rows: [] }));
      result.queries.location_log_company_ids = r6.rows.map(r => r.company_id);
    }

    // Test 7: check actual columns in trip_expenses
    const r7 = await pool.query(
      `SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'trip_expenses' ORDER BY ordinal_position`
    ).catch(e => ({ rows: [] }));
    result.queries.trip_expenses_columns = r7.rows.map(r => r.column_name);

    // Test 8: check actual columns in location_logs
    ...
    // Test 9: check users table columns
    const r9 = await pool.query(
      `SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'users' ORDER BY ordinal_position`
    ).catch(e => ({ rows: [] }));
    result.queries.users_columns = r9.rows.map(r => r.column_name);

    // Test 10: raw sample of expenses for this user
    if (user_id) {
      const r10 = await pool.query(
        `SELECT * FROM trip_expenses WHERE user_id::text = $1 LIMIT 2`,
        [String(user_id)]
      ).catch(e => ({ rows: [], error: e.message }));
      result.queries.raw_expense_sample = r10.rows || [];
      if (r10.error) result.queries.expense_error = r10.error;

      const r11 = await pool.query(
        `SELECT * FROM meetings WHERE user_id::text = $1 LIMIT 2`,
        [String(user_id)]
      ).catch(e => ({ rows: [], error: e.message }));
      result.queries.raw_meeting_sample = r11.rows || [];
      if (r11.error) result.queries.meeting_error = r11.error;

      const r12 = await pool.query(
        `SELECT id, latitude, longitude, timestamp, pincode, battery, activity FROM location_logs WHERE user_id::text = $1 LIMIT 2`,
        [String(user_id)]
      ).catch(e => ({ rows: [], error: e.message }));
      result.queries.raw_log_sample = r12.rows || [];
      if (r12.error) result.queries.log_error = r12.error;
    }
...
// ═══════════════════════════════════════════════════════════════
// USER MANAGEMENT — Bitrix-facing endpoints (uses member_id auth)
// ═══════════════════════════════════════════════════════════════

// ── User Management Launcher — GET|POST /bitrix/user-management ──
export const userManagementLauncher = async (req, res) => {
  ...
  try {
    ...
    const html = renderView("user.management.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── User Management Data — GET /bitrix/user-management-data ──
export const userManagementData = async (req, res) => {
  ...
  try {
    ...
    // Identify the portal admin (me) — the primary super admin of this company
...
// ── Create User — POST /bitrix/user-management-create ────────
export const userManagementCreate = async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ userManagementCreate: created ${email} for company ${company_id}`);
...
// ── Update User — POST /bitrix/user-management-update ────────
export const userManagementUpdate = async (req, res) => {
  ...
  try {
    ...
    if (userCheck.rows.length === 0) return res.status(404).json({ error: "UserNotFound", message: "User not found in your company" });

    if (email !== undefined) {
      const emailCheck = await pool.query("SELECT id FROM users WHERE email = $1 AND id != $2", [email, userId]);
      if (emailCheck.rows.length > 0) return res.status(409).json({ error: "EmailAlreadyExists", message: "Email already used by another user" });
    ...
    await pool.query(
      `INSERT INTO profiles (user_id, full_name, department, work_hours_start, work_hours_end)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         department       = COALESCE($3, profiles.department),
         work_hours_start = COALESCE($4, profiles.work_hours_start),
         work_hours_end   = COALESCE($5, profiles.work_hours_end)`,
      [userId, fullName || null, department || null, workHoursStart || null, workHoursEnd || null]
    );
    console.log(`✅ userManagementUpdate: updated user ${userId}`);
...
// ── Delete User — POST /bitrix/user-management-delete ────────
export const userManagementDelete = async (req, res) => {
  ...
  try {
    ...
    const userCheck = await pool.query(
      "SELECT id, email, is_super_admin FROM users WHERE id = $1 AND company_id = $2",
      [userId, company_id]
    );
    if (userCheck.rows.length === 0) return res.status(404).json({ error: "UserNotFound", message: "User not found in your company" });
    ...
    if (userCheck.rows[0].is_super_admin) {
      const superCount = await pool.query(
        "SELECT COUNT(*) FROM users WHERE company_id = $1 AND is_super_admin = true",
        [company_id]
      );
      if (parseInt(superCount.rows[0].count) <= 1) {
        return res.status(400).json({ error: "CannotDeleteLastAdmin", message: "Cannot delete the last administrator of the company" });
      }
    }

    const userEmail = userCheck.rows[0].email;
    await pool.query("DELETE FROM users WHERE id = $1", [userId]);
    console.log(`🗑️ userManagementDelete: deleted ${userEmail} (${userId})`);
...
// ── Reset Password — POST /bitrix/user-management-reset-password
export const userManagementResetPassword = async (req, res) => {
  ...
  try {
    ...
    if (!userId)      return res.status(400).json({ error: "userId required" });
    if (!newPassword || newPassword.length < 6) return res.status(400).json({ error: "PasswordTooShort", message: "Password must be at least 6 characters" });
    ...
    // Invalidate sessions (safe — ignore if table doesn't exist)
    try { await pool.query("DELETE FROM user_sessions WHERE user_id = $1", [userId]); } catch(_) {}

    console.log(`🔑 userManagementResetPassword: reset for ${userCheck.rows[0].email}`);
...
// ═══════════════════════════════════════════════════════════════
// BANK ACCOUNT — Bitrix-facing endpoints (uses member_id auth)
// Auto-creates user_bank_accounts table on first use
// ═══════════════════════════════════════════════════════════════
...
// ── Bank Account Launcher — GET|POST /bitrix/bank-account ─────
export const bankAccountLauncher = async (req, res) => {
  ...
  try {
    ...
    const html = renderView("bank.account.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Bank Account Data — GET /bitrix/bank-account-data ─────────
// Returns all users with their bank details (if saved)
export const bankAccountData = async (req, res) => {
  ...
  try {
    ...
    // All users with their bank account data (LEFT JOIN)
    const { rows: users } = await pool.query(
      `SELECT
         u.id,
         u.email,
       ...
       FROM users u
       LEFT JOIN profiles p ON p.user_id = u.id
       LEFT JOIN user_bank_accounts b ON b.user_id = u.id
       WHERE u.company_id = $1
    ...
    console.log(`✅ bankAccountData: ${users.length} users for company ${company_id}`);
    const planRes = await pool.query(
      "SELECT COALESCE(current_plan,'enterprise') AS current_plan FROM companies WHERE id = $1", [company_id]
    ).catch(() => ({ rows: [{ current_plan: 'enterprise' }] }));
    res.json({ company: company_name, users, me: admins[0] || null, current_plan: (planRes.rows[0]?.current_plan || 'enterprise').toLowerCase() });
...
// ── Save Bank Account — POST /bitrix/bank-account-save ────────
export const bankAccountSave = async (req, res) => {
  ...
  try {
    ...
    if (userCheck.rows.length === 0) {
      return res.status(404).json({ error: "UserNotFound", message: "User not found in your company" });
    }
    ...
    await pool.query(
      `INSERT INTO user_bank_accounts
         (user_id, company_id, holder_name, account_number, ifsc_code, bank_name, upi_id, updated_at)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         upi_id         = $7,
         updated_at     = now()`,
      [userId, company_id, holderName || null, accountNumber || null,
       ifscCode || null, bankName || null, upiId || null]
    );

    console.log(`✅ bankAccountSave: saved bank details for user ${userId}`);
...
// ═══════════════════════════════════════════════════════════════
// EXPAND CAPACITY — Bitrix-facing endpoints (uses member_id auth)
// Auto-creates slot_orders table on first use
// ═══════════════════════════════════════════════════════════════
...
async function ensureSlotOrdersTable() {
  ...
  `);
  // Ensure companies table has user_limit column
  try {
    await pool.query(`ALTER TABLE companies ADD COLUMN IF NOT EXISTS user_limit INTEGER DEFAULT 1000`);
  } catch(_) {}
}

// ── Expand Capacity Launcher — GET|POST /bitrix/expand-capacity ──
export const expandCapacityLauncher = async (req, res) => {
  ...
  try {
    ...
    const html = renderView("expand.capacity.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Expand Capacity Data — GET /bitrix/expand-capacity-data ───
export const expandCapacityData = async (req, res) => {
  ...
  try {
    ...
    const { rows: uRows } = await pool.query(
      "SELECT COUNT(*) AS cnt FROM users WHERE company_id = $1", [company_id]
    );
    // Client count
    const { rows: cRows } = await pool.query(
      "SELECT COUNT(*) AS cnt FROM clients WHERE company_id = $1", [company_id]
    );
    // User limit from companies table (with fallback)
    const { rows: limRows } = await pool.query(
      "SELECT COALESCE(user_limit, 1000) AS user_limit FROM companies WHERE id = $1", [company_id]
    ).catch(() => ({ rows: [{ user_limit: 1000 }] }));
    ...
    console.log(`✅ expandCapacityData: company ${company_id}`);
    const planRes = await pool.query(
      "SELECT COALESCE(current_plan,'enterprise') AS current_plan FROM companies WHERE id = $1", [company_id]
    ).catch(() => ({ rows: [{ current_plan: 'enterprise' }] }));
    res.json({
      company:      company_name,
      user_count:   parseInt(uRows[0].cnt),
      client_count: parseInt(cRows[0].cnt),
      user_limit:   parseInt(limRows[0]?.user_limit || 1000),
      me:           admins[0] || null,
      current_plan: (planRes.rows[0]?.current_plan || 'enterprise').toLowerCase(),
    });
  } catch(e) {
...
// ── Process Order — POST /bitrix/expand-capacity-order ────────
export const expandCapacityOrder = async (req, res) => {
  ...
  try {
    ...
    if (userSlots < 0 || clientSlots < 0) {
      return res.status(400).json({ error: "InvalidSlots", message: "Slot counts must be non-negative" });
    }
    if (userSlots === 0 && clientSlots === 0) {
      return res.status(400).json({ error: "NoSlots", message: "Please select at least 1 slot" });
    }
    ...
    // Save order record
    await pool.query(
      `INSERT INTO slot_orders (company_id, user_slots, client_slots, total_amount, payer_name, payer_email, payer_phone, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'completed')`,
      [company_id, userSlots, clientSlots, totalAmount, payerName || null, payerEmail || null, payerPhone || null]
    );

    // Increase the user_limit for the company
    let newUserLimit = 1000;
    if (userSlots > 0) {
      const upd = await pool.query(
        `UPDATE companies SET user_limit = COALESCE(user_limit, 1000) + $1 WHERE id = $2
         RETURNING COALESCE(user_limit, 1000) AS user_limit`,
        [userSlots, company_id]
      ).catch(() => null);
      if (upd?.rows?.length) newUserLimit = upd.rows[0].user_limit;
    }

    console.log(`✅ expandCapacityOrder: company ${company_id} +${userSlots} users +${clientSlots} clients, total ₹${totalAmount}`);
    res.json({
      message:        "OrderPlaced",
...
// ═══════════════════════════════════════════════════════════════
// BILLING PLANS — Bitrix-facing endpoints (uses member_id auth)
// ═══════════════════════════════════════════════════════════════
...
// ── Billing Plans Launcher — GET|POST /bitrix/billing-plans ───
export const billingPlansLauncher = async (req, res) => {
  ...
  try {
    ...
    const html = renderView("billing.plans.html", { member_id, DOMAIN });
    return res.send(html);
  } catch(e) {
...
// ── Billing Plans Data — GET /bitrix/billing-plans-data ───────
export const billingPlansData = async (req, res) => {
  ...
  try {
    ...
    // Ensure current_plan column exists on companies table
    await pool.query(`ALTER TABLE companies ADD COLUMN IF NOT EXISTS current_plan TEXT DEFAULT 'enterprise'`).catch(() => {});

    // Counts — all in parallel including services, expenses, location logs
    const [uRes, cRes, mRes, mDoneRes, limRes, svcRes, expRes, logRes, planRes] = await Promise.all([
      pool.query("SELECT COUNT(*) AS cnt FROM users    WHERE company_id = $1", [company_id]),
      ...
      pool.query("SELECT COALESCE(user_limit,1000) AS user_limit FROM companies WHERE id = $1", [company_id])
        .catch(() => ({ rows: [{ user_limit: 1000 }] })),
      pool.query("SELECT COUNT(*) AS cnt FROM client_services WHERE company_id = $1", [company_id])
        .catch(() => ({ rows: [{ cnt: 0 }] })),
      pool.query("SELECT COUNT(*) AS cnt FROM trip_expenses WHERE company_id = $1", [company_id])
        .catch(() => ({ rows: [{ cnt: 0 }] })),
      pool.query("SELECT COUNT(*) AS cnt FROM location_logs WHERE company_id = $1", [company_id])
        .catch(() => ({ rows: [{ cnt: 0 }] })),
      pool.query("SELECT COALESCE(current_plan,'enterprise') AS current_plan FROM companies WHERE id = $1", [company_id])
        .catch(() => ({ rows: [{ current_plan: 'enterprise' }] })),
    ]);

    // Synced clients count (from bitrix synced file)
    let synced_count = 0;
    try {
      const synced = loadSynced();
      const portalData = synced[member_id] || { clients: {} };
      synced_count = Object.keys(portalData.clients || {}).length;
    } catch(_) {}
    ...
    console.log(`✅ billingPlansData: company ${company_id}`);
    res.json({
      company:            company_name,
      current_plan:       (planRes.rows[0]?.current_plan || 'enterprise').toLowerCase(),
      user_count:         parseInt(uRes.rows[0].cnt),
      client_count:       parseInt(cRes.rows[0].cnt),
      user_limit:         parseInt(limRes.rows[0]?.user_limit || 1000),
      total_meetings:     parseInt(mRes.rows[0].cnt),
      completed_meetings: parseInt(mDoneRes.rows[0].cnt),
      total_services:     parseInt(svcRes.rows[0].cnt),
      total_expenses:     parseInt(expRes.rows[0].cnt),
      location_logs:      parseInt(logRes.rows[0].cnt),
      synced_count,
      me: admins[0] || null
    });
  } catch(e) {
...
// ── Plan Upgrade — POST /bitrix/billing-plans-upgrade ─────────
export const billingPlansUpgrade = async (req, res) => {
  ...
  try {
    ...
    if (!validPlans.includes(plan)) {
      return res.status(400).json({ error: "InvalidPlan", message: "Unknown plan key" });
    }

    await ensurePlanOrdersTable();

    // Record upgrade order
    await pool.query(
      `INSERT INTO plan_upgrade_orders (company_id, plan_key, payer_name, payer_email, payer_phone, status)
       VALUES ($1, $2, $3, $4, $5, 'completed')`,
      [company_id, plan, payerName || null, payerEmail || null, payerPhone || null]
    );

    // Update plan on companies table if column exists
    try {
    ...
    console.log(`✅ billingPlansUpgrade: company ${company_id} upgraded to ${plan}`);
...
// ═══════════════════════════════════════════════════════════════
// BITRIX ADMIN LOGIN — isolated from React production dashboard
// ═══════════════════════════════════════════════════════════════
//
// ISOLATION STRATEGY:
//   React production app:  localStorage.setItem("token", ...)
//   Bitrix admin session:  sessionStorage.setItem("bitrix_admin_token", ...)
//   → Completely separate storage — zero risk to production
//
// The login calls the SAME /auth/login endpoint as AdminLogin.js
// Server-side: we verify the JWT ourselves here (no new auth tables needed)
//

// ── Login Page Launcher — GET /bitrix/login ───────────────────
...
// ── Logout — GET /bitrix/logout ───────────────────────────────
// Clears the Bitrix session and redirects to login
// The React production localStorage.token is NEVER touched here
export const bitrixLogout = async (req, res) => {
  res.setHeader("ngrok-skip-browser-warning", "true");
  const DOMAIN    = req.query.DOMAIN    || "world.bitrix24.com";
  const member_id = req.query.member_id || "";
  // Token is stored in browser sessionStorage — cleared client-side
  // Server just redirects to login page
